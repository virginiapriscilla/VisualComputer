...
emotionsDataFile =   ['.\emotions\emotionsTDataFile.' date '_' int2str(randi(100,1)) '.xlsx'];
...
%fprintf(emotionsDataFID,"%s, %s, %s, %s, %s, %s, %s, %s\n", 'image name','emotions','ic_cc','ic_oc','ic_bc','cc_oc','cc_bc','oc_bc');
...
image_list=["image_0001.png" "image_0008.png" "image_0011.png" "image_0014.png" "image_0021.png" "image_0022.png" "image_0029.png" "image_0044.png" "image_0059.png" "image_0093.png" "image_0099.png" "image_0115.png"];
emotion_list=["Happy" "Happy" "Happy" "Happy" "Happy" "Happy" "Surprise" "Surprise" "Surprise" "Surprise" "Surprise" "Surprise"];

xlRange='A1:AYG1';
%xlswrite(emotionsDataFile,[ "images" "emotions" "distance type" [1:1330]], xlRange);
...
figure;imshow(input_image, []); hold on; plot(shapes_new (:,1), shapes_new (:,2), '.','MarkerSize',11);text(shapes_new(:,1),shapes_new(:,2),int2str(shapes_new(:,3)));
...
fetT_array_init = [ string(names1(gg).name) string(image_emotion) 'ic_cc'; string(names1(gg).name) string(image_emotion) 'ic_oc'; string(names1(gg).name) string(image_emotion) 'ic_bc'; string(names1(gg).name) string(image_emotion) 'cc_oc'; string(names1(gg).name) string(image_emotion) 'cc_bc'; string(names1(gg).name) string(image_emotion) 'oc_bc' ] ;

index_lower=strcat('A', int2str((excel_index)*excel_index_size+2));
index_upper=strcat('AYG', int2str((excel_index)*excel_index_size+1+7));
xlRange = strcat(index_lower,':',index_upper);
%xlswrite(emotionsDataFile,[ fetT_array_init  transpose(fetT_array(:,15:20)) ], xlRange);
